...
def densenet169_model(img_rows, img_cols, color_type=1, nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.5, dropout_rate=0.0, weight_decay=1e-4, num_classes=None):
    ...
    if K.image_dim_ordering() == 'th':
      ...
      weights_path = 'model/densenet_169_08-09_new_5classes'
...
if __name__ == '__main__':
    ...
    #x4_,y4_ = read_image('/home/daisy001/mountdir/qihaoyang/cnn_finetune-master_/Data-defect/shelling_new_','Shelling')
...
#        x2_train, x2_test, y2_train, y2_test = train_test_split(x2, y2, test_size=0.3, random_state=20)
        #x2_train_, x2_test_, y2_train_, y2_test_ = train_test_split(x2_, y2_, test_size=0.7, random_state=18)
...
#        #x5_train_, x5_test_, y5_train_, y5_test_ = train_test_split(x5_, y5_, test_size=0.3, random_state=20)
        x1_f_train, x1_f_test, y1_f_train, y1_f_test = train_test_split(x1_f, y1_f, test_size=0.5, random_state=18)
        x2_f_train, x2_f_test, y2_f_train, y2_f_test = train_test_split(x2_f, y2_f, test_size=0.5, random_state=18)
        x3_f_train, x3_f_test, y3_f_train, y3_f_test = train_test_split(x3_f, y3_f, test_size=0.5, random_state=18)
        x4_f_train, x4_f_test, y4_f_train, y4_f_test = train_test_split(x4_f, y4_f, test_size=0.5, random_state=18)
        x5_f_train, x5_f_test, y5_f_train, y5_f_test = train_test_split(x5_f, y5_f, test_size=0.5, random_state=18)
        #x1_o_train, x1_o_test, y1_o_train, y1_o_test = train_test_split(x1_o, y1_o, test_size=0.7, random_state=18)
        #x5_o_train, x5_o_test, y5_o_train, y5_o_test = train_test_split(x5_o, y5_o, test_size=0.7, random_state=18)
        def augmentation(x1_train,y1_train):
            x1_train_ = x1_train
            x1_train1 = x1_train
            x1_train2 = x1_train
        ...
        filepath = '/home/daisy001/mountdir/qihaoyang/cnn_finetune-master_/model/densenet_169_'+time.strftime("%m-%d",time.localtime())+'_all_fake_5classes'
