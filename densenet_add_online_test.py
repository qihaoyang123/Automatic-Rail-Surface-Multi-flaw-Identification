...
if __name__ == '__main__':
    ...
    for threshold in range(1,2):
        ...
        predictions_valid = model.predict(x_test_[0:len(x2_test_)])
        prediction = np.argmax(predictions_valid,axis=1)
        score1=accuracy_score(y2_test_, prediction)
        predictions_valid = model.predict(x_test_[len(x2_test_):len(x2_test_)+len(x1_o_test)])
        prediction = np.argmax(predictions_valid,axis=1)
        score2=accuracy_score(y1_o_test, prediction)
        predictions_valid = model.predict(x_test_[len(x2_test_)+len(x1_o_test):len(x2_test_)+len(x1_o_test)+len(x5_o_test)])
        prediction = np.argmax(predictions_valid,axis=1)
